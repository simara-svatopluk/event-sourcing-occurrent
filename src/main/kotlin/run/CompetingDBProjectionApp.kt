...
fun main() {
    ...
    database.createCollection("games-projection-competing")
    ...
    val subscriptionId = "games-durable-db-competing"
    ...
    val competingConsumerStrategy = NativeMongoLeaseCompetingConsumerStrategy
        .Builder(database)
        .leaseTime(1.seconds.toJavaDuration())
        .build()

    val competingConsumerSubscriptionModel =
        CompetingConsumerSubscriptionModel(modelToSubscribe, competingConsumerStrategy)


    val gamesCollection = database.getCollection("games-projection-competing")

    competingConsumerSubscriptionModel.subscribe(subscriptionId, filter(source(create("com.fairtiq.guessGame"))), startAt) { cloudEvent ->
